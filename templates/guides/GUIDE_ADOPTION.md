...
2. **Classify each template section against the project.** For every section of the chosen charter (and each requirement), decide and record one of:
...
3. **Conflicts: always ask.** When an existing instruction conflicts with a template practice, **stop and put it to the owner with a recommendation**. Neither side wins automatically — nothing is silently overwritten, and nothing is silently kept. This is the same Q&A-driven alignment the charters prescribe.
