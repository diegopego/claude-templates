...
- **`adopt-template/`** — bring these templates into a project that already exists, without losing its instructions: inventories the current `CLAUDE.md` and convention docs, classifies each charter section, chosen module, and requirement (keep / adapt / already-covered / skip), raises every conflict to the owner, and produces a merged `CLAUDE.md` plus seeded `.claude/memory/`. Handles the **upgrade** case too — a project already running an earlier version of these templates reconciles only the version diff. Automates `guides/GUIDE_ADOPTION.md`.
