...
- **Technical changelog** — for whoever maintains the code, curated **per significant change**, not per commit: the git history is the raw record underneath, and a changelog that restates it commit by commit is a hand-maintained duplicate that rots. Curated entries group and summarize that history. Each entry ends with a plain-language addendum for any jargon or dense passage.
