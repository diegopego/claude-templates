...
- **Multi-user, not one login per tenant.** A person belongs to **one or more** tenants, with a role *per* tenant — the same accountant serves three client companies; a shareholder of one is the CTO of another. So identity and authorization split cleanly: **one shared user base** (a person has a single account, whatever they belong to), and every permission hangs off the **membership** — user × tenant × role — never off the user record. Three consequences follow, and they are cheap now and expensive later: a session has a *current* tenant, not a permanent one; every domain query is scoped by membership; and inviting an existing user into a second tenant grants a membership, it does not create a second account. Which roles exist, and what each may do, is domain material — an Align question, not an inference.
