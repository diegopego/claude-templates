...
- Branch `master` (we always work directly on master — see `decisions.md`). This incorporation is **committed** — `9a39229` *Incorporate landing-publishing + the living-product-doc rewiring (cluster complete)* (16 files, pre-commit hook passed). Prior tip was `2574ce1`.
- **Marker note:** `.claude/memory/last-processed-commit.md` holds `2574ce1` (the parent of `9a39229`). By design it lags the doc-commit by one: the next `update-product-doc` run's window `2574ce1..HEAD` re-includes `9a39229`, which `README.md` already reflects → idempotent, then the marker advances. No action needed.
...
- The rendered **Artifact** landing was **republished to the same URL after owner approval** (2026-07-11; same URL `…/artifact/213558f8-…`, Nord skin unchanged; added a *A living landing page* practice card + landing-skin clauses in the Setup phase and New-project adopt cards). Canonical `README.md` is current. To update the Artifact again: fetch the URL via **WebFetch** (it returns the full raw HTML incl. the `<style>` skin — use WebFetch, not curl), edit only content, republish to the same URL after approval. Favicon set to 📜 (keep stable). The Artifact HTML is not versioned in the repo (by design).
