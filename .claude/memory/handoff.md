...
- Branch `master` (we always work directly on master — see `decisions.md`). The full change for this incorporation is **in the working tree but NOT committed** — it awaits the owner's per-instance git authorization (nothing was staged or committed this session). When authorizing, stage everything together so the pre-commit hook passes (see *Commit ritual* below).
...
- The rendered **Artifact landing was NOT republished** — held for owner approval per the landing approval loop (intent → preview → approval → publish). Canonical `README.md` is current.
