...
## Adoção só entrega o módulo living-docs, não os outros add-ons (fricção, 2026-07-11)

Para a skill `update-living-docs` chegar num adotante, o kit de `make adopt` passou a carregar `MODULE_LIVING_DOCS.md` + a skill pareada. Mas os outros módulos opt-in (`MODULE_PRODUCT_AUDIENCE`, `MODULE_DATA_MIGRATION`) continuam fora do kit de adoção — ele só entrega os charters compostos (mínimos) + requirement + guia. Ou seja, "adotar um módulo à la carte" (que o GUIDE promete) não é totalmente suportado: a fusão não tem o texto-fonte dos outros módulos para dobrar no CLAUDE.md. Solução de fundo: o kit de adoção deveria entregar `charters/sources/` inteiro (todos os módulos + manifesto) e as skills pareadas, não só o de living-docs. Fiz o recorte mínimo agora (living-docs, que era o pedido); generalizar fica para uma rodada.
