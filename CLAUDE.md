...
- `ideas/` — the idea pipeline. `inbox.md` is the owner's scratchpad (any language, draft quality; never reorganize or delete entries without being asked). On the owner's request an entry **graduates** into its own spec file — kebab-case, with `Status: draft | agreed | incorporated`, `Applies to`, and Behavior ("when X, the agent should Y") / Why / Example / Open questions sections — and is removed from the inbox in the same change. When graduating, the agent first runs a **Q&A round** on the entry: it surfaces every open question that would change the outcome and resolves it with the owner — questions batched, each with options and a recommendation, scripted-baseline-then-adaptive — and records the resolutions in `decisions.md`. Only then does it draft the spec, *Open questions* filled in and the settled ones already reflected; the spec reaches `agreed` once no outcome-changing question remains. With no outcome-changing questions, graduation proceeds straight through. This is the same **Q&A-round** method the charters prescribe (`CHARTER_CORE.md` → *Working through questions*) — the meta-project dogfoods it. Specs then graduate into template text.
